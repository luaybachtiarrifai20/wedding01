...
import bankMandiri from "../../images/bank-mandiri-seeklogo.png";
...
export const PageGift: React.FC = () => {
  ...
  const { bankAccounts, giftShipping } = weddingConfig.gift;
            ...
            {bankAccounts.map((account, index) => (
              ...
              >
                ...
                <div className="w-full flex justify-between items-start gap-4">
                  <img src={chipATM} alt="Chip ATM" className="w-[60px] h-[60px]" />
                  <img src={bankMandiri} alt={account.bankName} className="w-[100px] h-auto" />
                </div>

                {/* Masked account number */}
                <p
                  className="text-black text-[18px] font-semibold w-full text-left"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  {account.accountNumber.substring(0, 8)}xxxxx
                </p>

                {/* Account holder name */}
                <p
                  className="text-black text-[16px] w-full text-left"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  {account.accountName}
                </p>
                ...
                >
                  ...
                  {copiedIndex === index ? "Berhasil disalin ✓" : `Salin ${account.accountNumber}`}
                </button>
              </div>
            ))}
